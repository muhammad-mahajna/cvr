...
normalize_and_extract_roi() {
    ...
    TEMPLATE_IMAGE="$PREPROCESSING_DIR/rsBOLD_first_frame.nii.gz"  # use first frame as a template

    # Check if the required input files are available
    if [ ! -f "$TEMPLATE_IMAGE" ]; then
        echo "First Frame used as template - image not found for subject $SUBJECT_ID: $TEMPLATE_IMAGE" | tee -a "$LOG_FILE"
        return 1
    fi
